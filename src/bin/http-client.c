...
#ifdef _WIN32
static int add_cert_for_store(X509_STORE *store, const char *name)
{
	...
	while ((ctx = CertEnumCertificatesInStore(sys_store, ctx))) {
		X509 *x509 =
			d2i_X509(NULL,
				 (unsigned char const **)&ctx->pbCertEncoded,
				 ctx->cbCertEncoded);
		if (x509) {
...
int main(int argc, char **argv)
{
	...
	r = evhttp_make_request(
		evcon, req, data_file ? EVHTTP_REQ_POST : EVHTTP_REQ_GET, uri);
	if (r != 0) {
