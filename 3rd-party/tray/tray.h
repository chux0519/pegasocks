...
static int tray_init(struct tray *tray)
{
	...
	indicator =
		app_indicator_new(TRAY_APPINDICATOR_ID, tray->icon,
...
{
	id menu = objc_msgSend((id)objc_getClass("NSMenu"),
	...
	case WM_COMMAND:
		if (wparam >= ID_TRAY_FIRST) {
			...
			if (GetMenuItemInfo(hmenu, wparam, FALSE, &item)) {
				struct tray_menu *menu =
					(struct tray_menu *)item.dwItemData;
...
static HMENU _tray_menu(struct tray_menu *m, UINT *id)
{
	...
	for (; m != NULL && m->text != NULL; m++, (*id)++) {
		if (strcmp(m->text, "-") == 0) {
			...
			item.fMask =
				MIIM_ID | MIIM_TYPE | MIIM_STATE | MIIM_DATA;
			item.fType = 0;
			item.fState = 0;
			if (m->submenu != NULL) {
...
static int tray_init(struct tray *tray)
{
	...
	hwnd = CreateWindowEx(0, WC_TRAY_CLASS_NAME, NULL, 0, 0, 0, 0, 0, 0, 0,
			      0, 0);
	if (hwnd == NULL) {
		return -1;
	}
